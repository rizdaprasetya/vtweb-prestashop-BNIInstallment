...
class VeritransBniValidationModuleFrontController extends ModuleFrontController
{
  ...
  //   $veritrans->version = Configuration::get('VN_API_VERSION');
  //   $veritrans->environment = Configuration::get('VN_ENVIRONMENT');
  //   $veritrans->payment_type = Configuration::get('VN_PAYMENT_TYPE') == 'vtdirect' ? Veritrans_Config::VN_DIRECT : Veritrans_Config::VN_WEB;
  //   $veritrans->merchant_id = Configuration::get('VN_MERCHANT_ID');
  //   $veritrans->merchant_hash_key = Configuration::get('VN_MERCHANT_HASH');
  //   $veritrans->client_key = Configuration::get('VN_CLIENT_KEY');
  //   $veritrans->server_key = Configuration::get('VN_SERVER_KEY');
  //   $veritrans->enable_3d_secure = Configuration::get('VN_3D_SECURE');
  ...
  //   $this->module->validateOrder($cart->id, Configuration::get('VN_ORDER_STATE_ID'), $cart->getOrderTotal(true, Cart::BOTH), $this->module->displayName, NULL, $mailVars, (int)$currency->id, false, $customer->secure_key);
  ...
  if ($veritrans_api_version == 2 && $veritrans_payment_method == 'vtweb')
  {
      if ($keys['isWarning']){          
          ...
          Tools::redirectLink('index.php?fc=module&module=veritransbni&controller=warning&redirlink='.$keys['redirect_url'].'&message='.$keys['message']);
      }      
