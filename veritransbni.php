...
class VeritransBni extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$this->config_keys = array(			
			...
			'VN_KURS',
			'VN_CONVENIENCE_FEE',
			'VN_PAYMENT_SUCCESS_STATUS_MAP',
			'VN_PAYMENT_FAILURE_STATUS_MAP',
			'VN_PAYMENT_CHALLENGE_STATUS_MAP',
			'VN_ENVIRONMENT',
			'VN_SANITIZED',
	...
	public function install()
	{
		...
		$order_state->module_name = 'veritransbni';
		...
		Configuration::updateValue('VN_API_VERSION', 2);
		...
		include_once(_PS_MODULE_DIR_ . '/' . $this->name . '/vtbni_install.php');
		$vtbni_install = new VeritransBniInstall();
		$vtbni_install->createTable();
	...
	private function _postValidation()
	{
		...
		{
			if (Tools::getValue('VN_API_VERSION') == 2)
			{
				if (!Tools::getValue('VN_SERVER_KEY'))
					$this->_postErrors[] = $this->l('Server Key is required.');
			} 

			// validate conversion rate existence
			if (!Currency::exists('IDR', null) && !Tools::getValue('VN_KURS'))
			{
	...
	private function _displayVeritransBni()
	{
	...
	private function _displayVeritransBniOld()
	{
		$this->_html .= '<img src="../modules/veritransbni/Veritrans.png" style="float:left; margin-right:15px;"><b>'.$this->l('Extends Veritrans payment method to accept payment using BNI Installments.').'</b><br/><br/>
		'.$this->l('Payment BNI Installments via veritrans.').'<br /><br /><br />';
	}
		...
		$fields_form = array(
			'form' => array(
				...
				'input' => array(
					...
					array(
						'type' => 'text',
						'label' => 'BNI Installment terms',
						'name' => 'VN_INSTALLMENTS_BNI',
						'desc' => 'Insert installment terms in months, separate using coma (,) for multiple value. e.g: 3,6,12',
						//'class' => 'v1_vtweb_settings sensitive'\
						'class' => 'VN_INSTALLMENTS_BNI'	
						),
					// array(
					// 	'type' => (version_compare(Configuration::get('PS_VERSION_DB'), '1.6') == -1)?'radio':'switch',
					...
					// 	'name' => 'VN_INSTALLMENTS_MANDIRI',
					// 	//'class' => 'v1_vtweb_settings sensitive'\
					// 	'class' => 'VN_INSTALLMENTS_MANDIRI'	
					// 	),
				/*	array(
						'type' => 'checkbox',
						'label' => 'Enable Mandiri Installments?',
						'name' => 'VN_INSTALLMENTS',
						'values' => array(
					...
					array(
						...
						'name' => 'VN_PAYMENT_SUCCESS_STATUS_MAP',
						'required' => true,	
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
					...
					array(
						...
						'name' => 'VN_PAYMENT_FAILURE_STATUS_MAP',
						'required' => true,
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
					...
					array(
						...
						'name' => 'VN_PAYMENT_CHALLENGE_STATUS_MAP',
						'required' => true,
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
	...
	private function _displayFormOld()
	{
		...
		$this->context->smarty->assign(array(
			...
			'api_version' => htmlentities(Configuration::get('VN_API_VERSION'), ENT_COMPAT, 'UTF-8'),
			'api_versions' => array(1 => 'v1', 2 => 'v2'),
			//'payment_type' => htmlentities(Configuration::get('VN_PAYMENT_TYPE'), ENT_COMPAT, 'UTF-8'),
			//'payment_types' => array('vtweb' => 'VT-Web', 'vtdirect' => 'VT-Direct'),
			// 'client_key' => htmlentities(Configuration::get('VN_CLIENT_KEY'), ENT_COMPAT, 'UTF-8'),
			'server_key' => htmlentities(Configuration::get('VN_SERVER_KEY'), ENT_COMPAT, 'UTF-8'),
			'environments' => array(false => 'Development', true => 'Production'),
			'environment' => htmlentities(Configuration::get('VN_ENVIRONMENT'), ENT_COMPAT, 'UTF-8'),
			// 'enable_3d_secure' => htmlentities(Configuration::get('VN_3D_SECURE'), ENT_COMPAT, 'UTF-8'),
			// 'enable_sanitized' => htmlentities(Configuration::get('VN_SANITIZED'), ENT_COMPAT, 'UTF-8'),
			...
			'payment_success_status_map' => htmlentities(Configuration::get('VN_PAYMENT_SUCCESS_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'payment_challenge_status_map' => htmlentities(Configuration::get('VN_PAYMENT_CHALLENGE_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'payment_failure_status_map' => htmlentities(Configuration::get('VN_PAYMENT_FAILURE_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'kurs' => htmlentities(Configuration::get('VN_KURS', $this->veritrans_kurs), ENT_COMPAT, 'UTF-8'),
			//'kurs' => htmlentities(Configuration::get('VN_INSTALLMENTS_BNI', ENT_COMPAT, 'UTF-8'),
			'convenience_fee' => htmlentities(Configuration::get('VN_CONVENIENCE_FEE', $this->veritrans_convenience_fee), ENT_COMPAT, 'UTF-8'),
	...
	public function execPayment($cart)
	{
		...
		$smarty->assign(array(
			'payment_type' => Configuration::get('VN_PAYMENT_TYPE'),
			'api_version' => Configuration::get('VN_API_VERSION'),
			'error_message' => '',
			'link' => $link,
...
//
	public function getTermInstallment($name_bank){
		$ans = array();
		foreach ($this->config_keys as $key) {
			if ( (strpos($key, 'VN_INSTALLMENTS_' . $name_bank ) !== FALSE) && (Configuration::get($key) == 'on') ){
				
				$term = Configuration::get('VN_INSTALLMENTS_'.$name_bank);
	...
	public function execValidation($cart)
	{
		...
		$usd = Configuration::get('VN_KURS');
		$cf = Configuration::get('VN_CONVENIENCE_FEE') * 0.01;
   ...
   //  	if (Configuration::get('VN_3D_SECURE') == 'on' || Configuration::get('VN_3D_SECURE') == 1)
		...
		//error_log('sanitized '.Configuration::get('VN_SANITIZED'));
		...
		if ($cart_currency->iso_code != 'IDR')
		{
			...
			{
			  // use rate
			  $conversion_func = function($input) { return $input * intval(Configuration::get('VN_KURS')); };
			}
		...
		$installment_type_val = Configuration::get('VN_ENABLE_INSTALLMENT');
		$param_required;
		switch ($installment_type_val) {
			case 'all_product':
				...
				if ($isBniInstallment){					
					...
					$a = Configuration::get('VN_INSTALLMENTS_BNI');
					$term = explode(',',$a);
					$bni_term = $term;
				...
				if ($isMandiriInstallment){
					...
					$a = Configuration::get('VN_INSTALLMENTS_MANDIRI');
					$term = explode(',',$a);
					$mandiri_term = $term;
	...
	//error_log($param_installment,true);
		...
		//BNI bin Filter
		$binarray[] = 'bni';
		if (Configuration::get('VN_ENVIRONMENT') == 'development'){
			$binarray[] = '4';
			$binarray[] = '5';
		}
		$params_all['vtweb']['credit_card_bins'] = $binarray;
		//end of BNI bin Filter
		
		// error_log(print_r(json_encode($params_all),true)); // debugan
		
		if (Configuration::get('VN_API_VERSION') == 2 && Configuration::get('VN_PAYMENT_TYPE') != 'vtdirect') //transaksi https://github.com/veritrans/veritrans-php/blob/vtweb-2/examples/v2/VN_web/checkout_process.php line 77
		{						
			try {
